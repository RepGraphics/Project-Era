$(document).ready(function() {
  if (window.location.pathname.includes('/PlayStation-Official.html')) {
    $('#search').on('click', function() {
      const query = $('#search-bar').val().toLowerCase();
      $.ajax({
        method: "GET",
        url: 'https://corsproxy.io/?http://arkdedicated.com/sotfps4/cache/officialserverlist.json',
        dataType: 'json',
         success: function(data) {
          ...
          filteredData.forEach(function(server) {
            const tr = $('<tr>').append(
              $('<td>').text(server.Name.replace(/ - \(v\d+\.\d+\)$/, '')),
